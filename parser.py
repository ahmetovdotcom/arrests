...
def parse(file_path):
    ...
    # Уникальный номер
    unique_number = re.search(r'Уникальный номер\s+([0-9\- /]+)', full_text)
    unique_number = clean(unique_number.group(1)) if unique_number else None

    # Дата составления
    date_created_match = re.search(r'«(\d{1,2})»\s+([а-яА-ЯёЁ]+)\s+(\d{4})г', full_text)
    date_created = convert_date_format(date_created_match.group(1), date_created_match.group(2), date_created_match.group(3)) if date_created_match else None
